...
namespace itk.simple.examples
{
    class LandmarkRegistration
    {
        static void Main(string[] args)
        {
            ...
            for (uint x = 11; x < 20; ++x)
            {
                for (uint y = 11; y < 20; ++y)
                {
                    VectorUInt32 idx = new VectorUInt32(new uint[] {x, y});
                    fixedImage.SetPixelAsUInt8(idx, 200);
                }
            }
            ...
            for (uint x = 51; x < 60; ++x)
            {
                for (uint y = 51; y < 60; ++y)
                {
                    VectorUInt32 idx = new VectorUInt32(new uint[] {x, y});
                    movingImage.SetPixelAsUInt8(idx, 69);
                }
            }
