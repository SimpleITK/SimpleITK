...
ConvertImage(const std::string & inputFile, const std::string & outputFile, unsigned int newWidth = 0)
{
  try
  {
    ...
    auto                      iSize = reader.GetSize();
    std::vector<unsigned int> imageSize;
    imageSize.push_back(static_cast<unsigned int>(iSize[0]));
    imageSize.push_back(static_cast<unsigned int>(iSize[1]));
    // If 3D image with single slice, treat as 2D
    if (iSize.size() == 3)
    {
      if (iSize[2] == 1)
      {
        imageSize.push_back(0U);
      }
      else
      {
    ...
    if (image.GetNumberOfComponentsPerPixel() == 1)
    {
      ...
      std::string photoInterp = reader.GetMetaData("0028|0004");
