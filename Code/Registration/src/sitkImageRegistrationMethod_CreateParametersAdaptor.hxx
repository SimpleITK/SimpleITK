...
template <typename TTransformBase, typename TFixedImageType, typename TTransformType>
typename TransformParametersAdaptorBase<TTransformBase>::Pointer
CreateTransformParametersAdaptorBSpline(
  TTransformType *                                                      bsplineTransform,
...
ImageRegistrationMethod::CreateTransformParametersAdaptor(TRegistrationMethod * method)
{
  ...
  {
    const typename TRegistrationMethod::ShrinkFactorsPerDimensionContainerType & shrinkFactors =
      method->GetShrinkFactorsPerDimension(level);
    ...
    {
      adaptor = CreateTransformParametersAdaptorDisplacementField<TransformType>(
        displacementField, shrinkFactors, fixedImage, level);
    }
    else if (bsplineO3 != nullptr)
    {
      adaptor = CreateTransformParametersAdaptorBSpline<TransformType>(
        bsplineO3, shrinkFactors, fixedImage, bsplineScaleFactor);
    }
    else if (bsplineO2 != nullptr)
    {
      adaptor = CreateTransformParametersAdaptorBSpline<TransformType>(
        bsplineO2, shrinkFactors, fixedImage, bsplineScaleFactor);
    }
