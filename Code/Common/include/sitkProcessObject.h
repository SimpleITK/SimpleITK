 ...
 *
 */
class SITKCommon_EXPORT ProcessObject : protected NonCopyable
{
public:
   ...
   */
  static bool
  SetGlobalDefaultThreader(const std::string & threader);
  static std::string
   ...
   */
  virtual void
  SetNumberOfWorkUnits(unsigned int n);
  virtual unsigned int
   ...
   */
  virtual int
  AddCommand(itk::simple::EventEnum event, itk::simple::Command & cmd);
   ...
   */
  virtual int
  AddCommand(itk::simple::EventEnum event, const std::function<void()> & func);
#endif
  ...
  virtual bool
  HasCommand(itk::simple::EventEnum event) const;
...
protected:
  ...
  struct EventCommand
  {
    ...
    inline bool
    operator==(const EventCommand & o) const
    {
  ...
  virtual void
  onCommandDelete(const itk::simple::Command * cmd) noexcept;
#endif


  template <class TImageType>
  static typename TImageType::ConstPointer
  CastImageToITK(const Image & img)
  {
    typename TImageType::ConstPointer itkImage = dynamic_cast<const TImageType *>(img.GetITKBase());

    if (itkImage.IsNull())
    {
      sitkExceptionMacro("Failure to convert SimpleITK image of dimension: "
                         << img.GetDimension() << " and pixel type: \"" << img.GetPixelIDTypeAsString()
                         << "\" to ITK image of dimension: " << TImageType::GetImageDimension() << " and pixel type: \""
  ...
  static Image
  CastITKToImage(itk::Image<TVector<VLength>, VImageDimension> * img)
  {
    // The implementation defined int sitkImageConvert.hxx needs
    // to be manually included when this method is used.
    auto out = GetVectorImageFromImage(img, true);
  ...
  template <typename T>
  static std::ostream &
  ToStringHelper(std::ostream & os, const T & v)
  {
    os << v;
    return os;
  }
  static std::ostream &
  ToStringHelper(std::ostream & os, const char & v);
  static std::ostream &
  ToStringHelper(std::ostream & os, const signed char & v);
  static std::ostream &
  ToStringHelper(std::ostream & os, const unsigned char & v);
  /**@}*/
