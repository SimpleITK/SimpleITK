...
void
Transform::InternalBSplineInitialization(Image & inImage)
{
...
void
Transform::InternalDisplacementInitialization(Image & inImage)
{
...
void
Transform::SetPimpleTransform(std::unique_ptr<PimpleTransformBase> && pimpleTransform)
{
...
void
Transform::InternalInitialization(TransformEnum type, itk::TransformBase * base)
{
...
template void SITKCommon_EXPORT
Transform::InternalInitialization<2>(TransformEnum, itk::TransformBase *);
template void SITKCommon_EXPORT
Transform::InternalInitialization<3>(TransformEnum, itk::TransformBase *);

itk::TransformBase *
Transform::GetITKBase()
{
  assert(m_PimpleTransform);
...
void
Transform::SetParameters(const std::vector<double> & parameters)
{
  assert(m_PimpleTransform);
  this->MakeUnique();
...
unsigned int
Transform::GetNumberOfParameters() const
{
  assert(m_PimpleTransform);
...
void
Transform::SetFixedParameters(const std::vector<double> & parameters)
{
  assert(m_PimpleTransform);
  this->MakeUnique();
...
std::vector<double>
Transform::GetFixedParameters() const
{
  assert(m_PimpleTransform);
...
unsigned int
Transform::GetNumberOfFixedParameters() const
{
  assert(m_PimpleTransform);
...
std::vector<double>
Transform::TransformPoint(const std::vector<double> & point) const
{
  assert(m_PimpleTransform);
...
std::vector<double>
Transform::TransformVector(const std::vector<double> & vector, const std::vector<double> & point) const
{
  assert(m_PimpleTransform);
...
TransformEnum
Transform::GetTransformEnum() const
{
  assert(m_PimpleTransform);
...
void
Transform::InternalInitialization(itk::TransformBase * transform)
{
  ...
  {
    if (transform->GetInputSpaceDimension() == 2 && transform->GetInputSpaceDimension() == 2)
    {
      this->InternalInitialization<2>(sitkComposite, transform);
    }
    else if (transform->GetInputSpaceDimension() == 3 && transform->GetInputSpaceDimension() == 3)
    {
      this->InternalInitialization<3>(sitkComposite, transform);
    }
    else
    {
      sitkExceptionMacro("Unable to create transform with InputSpaceDimension: "
                         << transform->GetInputSpaceDimension()
...
void
Transform::InternalInitialization(TransformType * t)
{
...
ReadTransform(const PathType & filename)
{
  ...
  if (list->empty())
  {
    sitkExceptionMacro("Read transform file: \"" << filename
                                                 << "\", but there appears to be not transform in the file!");
  }
  ...
  if (list->front()->GetInputSpaceDimension() == 3 && list->front()->GetOutputSpaceDimension() == 3)
  {
  ...
  if (list->front()->GetInputSpaceDimension() == 2 && list->front()->GetOutputSpaceDimension() == 2)
  {
  ...
  sitkExceptionMacro("Unable to transform with InputSpaceDimension: "
                     << list->front()->GetInputSpaceDimension()
                     << " and OutputSpaceDimension: " << list->front()->GetOutputSpaceDimension() << ". "
                     << "Transform of type " << list->front()->GetNameOfClass() << "is not supported.");
}

void
Transform::WriteTransform(const std::string & filename) const
{
