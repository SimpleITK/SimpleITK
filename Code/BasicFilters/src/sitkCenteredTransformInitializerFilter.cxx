...
CenteredTransformInitializerFilter::GetMemberFactory()
{
...
//
Transform
CenteredTransformInitializerFilter::Execute(const Image &     fixedImage,
                                            ...
                                            const Transform & transform)
{
  ...
  if (!GetMemberFactory().HasMemberFunction(type, dimension))
  {
    sitkExceptionMacro("Filter does not support image type: " << GetPixelIDValueAsString(type) << " with dimension "
                                                              << dimension << ".");
  }
  return GetMemberFactory().GetMemberFunction(type, dimension, this)(&fixedImage, &movingImage, &transform);
}
...
Transform
CenteredTransformInitializerFilter::ExecuteInternal(const Image *     inFixedImage,
