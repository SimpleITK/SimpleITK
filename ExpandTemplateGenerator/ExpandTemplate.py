...
def expand_template(config_file:Path, template:Path, template_dirs: List[Path], output_file:Path, verbose= False, clobber:bool=True):
    ...
    # check all paths in template_dirs exist, print warning if they do not
    for template_dir in template_dirs:
        if not os.path.exists(template_dir):
            logging.warning(f"Template directory {template_dir} does not exist.")

    # add template_dirs to the Environment loader
    template_dirs = [Path(template_dir) for template_dir in template_dirs]

    env = Environment(loader=FileSystemLoader(template_dirs))
    # Add the regex_replace filter to the environment
    env.filters['regex_replace'] = regex_replace
    # Add the ruby_capitalize filter to the environment
    env.filters['ruby_capitalize'] = ruby_capitalize
    # Add the quote_string filter to the environment
    env.filters['quote'] = quote_string
    # Add the format_list filter to the environment
...
def main() -> int:
    ...
    # add option to make jina more verbose
