...
Building a C++ SimpleITK application is normally done using `CMake <https://cmake.org>`_ to
...
text file there are several variables needed to build a SimpleITK program, SimpleITK_DIR and ITK_DIR.
...
If you have built SimpleITK using the SuperBuild process, as described in :doc:`building`, then
...
#####################
Your program can be built by running cmake.  Here is the command to perform the build in the
current directory::
