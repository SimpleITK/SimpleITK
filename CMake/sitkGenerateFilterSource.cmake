...
if(NOT SimpleITK_LUA_EXECUTABLE)
  ...
  set(SimpleITK_LUA_EXECUTABLE
      ${LUA_EXECUTABLE}
      CACHE PATH "Lua executable used for code generation.")

  if(DEFINED SAVE_LUA_EXECUTABLE)
    set(LUA_EXECUTABLE
        ${SAVE_LUA_EXECUTABLE}
        CACHE ${SAVE_LUA_EXECUTABLE_TYPE} ${SAVE_LUA_EXECUTABLE_DOCSTRING} FORCE)
  else()
    unset(LUA_EXECUTABLE CACHE)
  endif()
endif()
...
execute_process(
  ...
  OUTPUT_VARIABLE SimpleITK_LUA_EXECUTABLE_VERSION_STRING
  ERROR_VARIABLE SimpleITK_LUA_EXECUTABLE_VERSION_STRING
  RESULT_VARIABLE SITK_LUA_VERSION_RESULT_VARIABLE
...
# Sets "out_var" variable name to the value in the json path specified to the json file name. If an error is encountered
# than the variable is not updated.
#
function(
  get_json_path
  out_var
  ...
  execute_process(
    COMMAND ${SimpleITK_LUA_EXECUTABLE} ${SimpleITK_SOURCE_DIR}/ExpandTemplateGenerator/JSONQuery.lua ${json_file}
            ${path}
...
# ######################################################################################################################
# This macro returns the list of template components used by a specific template file
#
macro(
  get_dependent_template_components
  out_var_name
  json_file
  input_dir)
  ...
  if(template_code_filename)
    ...
    if(NOT ${cache_var})
      ...
      foreach(component ${template_components})
        ...
        if("${h_contents}" MATCHES ".*${filename}.*" OR "${cxx_contents}" MATCHES ".*${filename}.*")
...
function(
  ...
  set(IMAGE_FILTER_LIST
      ${IMAGE_FILTER_LIST} ${FILENAME}
      CACHE INTERNAL "")
  ...
  if(Python_EXECUTABLE
     AND NOT
         ...
         VERSION_LESS
         2.6)
    set(JSON_VALIDATE_COMMAND COMMAND "${Python_EXECUTABLE}" "${SimpleITK_SOURCE_DIR}/Utilities/JSON/JSONValidate.py"
                              "${input_json_file}")
  endif()

  # header
  add_custom_command(
    OUTPUT "${output_h}" ${JSON_VALIDATE_COMMAND}
    COMMAND ${CMAKE_COMMAND} -E remove -f ${output_h}
    COMMAND ${SimpleITK_LUA_EXECUTABLE} ${expand_template_script} code ${input_json_file} ${input_dir}/templates/sitk
            ${template_include_dir} Template.h.in ${output_h}
  ...
  add_custom_command(
    ...
    COMMAND ${SimpleITK_LUA_EXECUTABLE} ${expand_template_script} code ${input_json_file} ${input_dir}/templates/sitk
            ${template_include_dir} Template.cxx.in ${output_cxx}
    DEPENDS ${input_json_file} ${template_deps} ${template_file_cxx})

  set(${library_name}GeneratedHeader
      ${${library_name}GeneratedHeader} "${output_h}"
      CACHE INTERNAL "")

  set(${library_name}GeneratedSource
      ${${library_name}GeneratedSource} "${output_cxx}"
      CACHE INTERNAL "")
  ...
  set(GENERATED_FILTER_LIST
      ${GENERATED_FILTER_LIST} ${FILENAME}
      CACHE INTERNAL "")
endfunction()
...
macro(generate_filter_source)
  ...
  set(SimpleITK${directory_name}GeneratedSource
      ""
      CACHE INTERNAL "")
  set(SimpleITK${directory_name}GeneratedHeader
      ""
      CACHE INTERNAL "")
  ...
  #
  add_custom_target(${directory_name}SourceCode ALL DEPENDS ${SimpleITK${directory_name}GeneratedHeader}
  ...
  install(
    FILES ${SimpleITK${directory_name}GeneratedHeader} ${generated_headers_h} ${generated_headers_i}
