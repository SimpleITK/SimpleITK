#
# This CMake code extracts the information from the git repository, and automatically causes a reconfigure if the git
# HEAD changes. The following variable may be defined after execution:
#
# _GIT_VERSION_HASH - the SHA1 hash of the current HEAD
#
# Based on the most recent tag starting with the letter "v" for version, which is expected to be of the form
# vN.N[.N[.N][(a|b|c|rc[N])] the following is extracted or undefined:
#
# _GIT_VERSION_MAJOR _GIT_VERSION_MINOR _GIT_VERSION_PATCH _GIT_VERSION_TWEAK _GIT_VERSION_RC
#
# If the current project's version ( defiend by ${CMAKE_PROJECT_NAME}_VERSION_MAJOR and MINOR and PATCH and TWEAK match
# that of the tag, then it'll be considered that the project is in post release mode otherwise it's considered
# underdevelopment.
#
# Only one of the following variables will be defined. _GIT_VERSION_DEV is defined as number of commits since the
# projects Version.cmake file has been modified. While _GIT_VERSION_POST is defined as the number of commits since the
# tag.
#
...
# if there is not a git directory we should be in a distributed package which should contain this additional cmake file
# with the _GIT_VERSION variables
...
if(_GIT_VERSION_HASH MATCHES "[a-fA-F0-9]+")
  string(
    SUBSTRING "${_GIT_VERSION_HASH}"
              0
              5
              _GIT_VERSION_HASH)
endif()

# Find the closest annotated tag with the v prefix for version. Consider all tags starting with v, to accommodate GitHub
# Actions way in which annotated tags are clobbered. See actions/checkout#290 for details.
...
if(_GIT_VERSION VERSION_EQUAL _${CMAKE_PROJECT_NAME}_VERSION)
  ...
  # The first commit after a tag should increase the project version number in Version.cmake and be "dev1"
