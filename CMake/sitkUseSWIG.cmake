# * SWIG module for CMake Defines the following macros: SWIG_ADD_MODULE(name language [ files ]) - Define swig module
#   with given name and specified language SWIG_LINK_LIBRARIES(name [ libraries ]) - Link libraries to swig module All
#   other macros are for internal use only. To get the actual target name of the swig moduel, use:
#   ${SWIG_MODULE_${name}_TARGET_NAME}. The target name can be used to set dependecies between targets. Set Source files
#   properties such as CPLUSPLUS and SWIG_FLAGS to specify special behavior of SWIG. Also global CMAKE_SWIG_FLAGS can be
#   used to add special flags to all swig calls. Another special variable is CMAKE_SWIG_OUTDIR, it allows one to specify
#   where to write all the swig generated module (swig -outdir option) The name-specific variable
#   SWIG_MODULE_<name>_EXTRA_DEPS may be used to specify extra dependencies for the generated modules. If the source
#   file generated by swig need some special flag you can use SET_SOURCE_FILES_PROPERTIES(
#   ${swig_generated_file_fullname} PROPERTIES COMPILE_FLAGS "-bla")

# =============================================================================
# Copyright 2004-2009 Kitware, Inc. Copyright 2009 Mathieu Malaterre <mathieu.malaterre@gmail.com>
#
...
#
# For a given language, input file, and output file, determine extra files that will be generated. This is internal swig
# macro.
#
...
macro(
  ...
  swig_get_extra_output_files(
    ${SWIG_MODULE_${name}_LANGUAGE}
    swig_extra_generated_files
    "${swig_outdir}"
    "${infile}")
  set(swig_generated_file_fullname "${swig_generated_file_fullname}/${swig_source_file_name_we}")
  # add the language into the name of the file (i.e. TCL_wrap) this allows for the same .i file to be wrapped into
  # different languages
  ...
  # MESSAGE("Full path to source file: ${swig_source_file_fullname}") MESSAGE("Full path to the output file:
  ...
  add_custom_command(
    ...
    COMMAND
      "${SWIG_EXECUTABLE}" ARGS "-${SWIG_MODULE_${name}_SWIG_LANGUAGE_FLAG}" ${swig_source_file_flags}
      ${CMAKE_SWIG_FLAGS} -outdir ${swig_outdir} ${swig_special_flags} ${swig_extra_flags} ${swig_include_dirs} -o
      "${swig_generated_file_fullname}" "${swig_source_file_fullname}"
    MAIN_DEPENDENCY "${swig_source_file_fullname}"
    "USES_TERMINAL"
    DEPENDS ${SWIG_MODULE_${name}_EXTRA_DEPS}
    COMMENT "Swig source to generate ${SWIG_MODULE_${name}_LANGUAGE} wrapping")
  set_source_files_properties("${swig_generated_file_fullname}" ${swig_extra_generated_files} PROPERTIES GENERATED 1)
...
macro(SWIG_ADD_MODULE name language)
  ...
  add_library(${SWIG_MODULE_${name}_TARGET_NAME} MODULE ${swig_generated_sources} ${swig_other_sources})
  set_target_properties(${SWIG_MODULE_${name}_TARGET_NAME} PROPERTIES OUTPUT_NAME "${name}")
  string(TOLOWER "${language}" swig_lowercase_language)
  if("${swig_lowercase_language}" STREQUAL "java")
    if(APPLE)
      # In java you want: System.loadLibrary("LIBRARY"); then JNI will look for a library whose name is platform
      # dependent, namely MacOS  : libLIBRARY.jnilib Windows: LIBRARY.dll Linux  : libLIBRARY.so
  ...
  if("${swig_lowercase_language}" STREQUAL "python")
    ...
    # when swig is used without the -interface it will produce in the module.py a 'import _modulename' statement, which
    # implies having a corresponding _modulename.so (*NIX), _modulename.pyd (Win32).
    ...
    # Python extension modules on Windows must have the extension ".pyd" instead of ".dll" as of Python 2.5.  Older
    # python versions do support this suffix. http://docs.python.org/whatsnew/ports.html#SECTION0001510000000000000000
    # <quote> Windows: .dll is no longer supported as a filename extension for extension modules. .pyd is now the only
    # filename extension that will be searched for. </quote>
