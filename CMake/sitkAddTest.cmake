...
# -----------------------------------------------------------------------------
# SimpleITK wrapper for add_test that adds support for external data, and post comparison processes.
#
# ARGUMENTS
#
# [ADD_TEST ARGUMENTS...] The first arguments are passed to cmake's add_test function with support for the DATA{}
# references to external data TRANSFORM_COMPARE <test transform> <baseline displacement> [tolerance] IMAGE_COMPARE <test
# image> <baseline image> [tolerance] IMAGE_MD5_COMPARE <test image> <md5 hash>
function(sitk_add_test)
  ...
  if(SimpleITK_TESTING_NOSHOW)
    set_property(TEST ${__NAME} PROPERTY ENVIRONMENT SITK_NOSHOW=YES)
  endif()
...
function(sitk_add_python_test name)
  ...
  sitk_add_test(
    ...
    ${command}
    ${ARGN})
  set_property(TEST Python.${name} PROPERTY LABELS Python)
  if(NOT SimpleITK_PYTHON_USE_VIRTUALENV)
    set_property(
      TEST Python.${name}
      APPEND
      PROPERTY ENVIRONMENT PYTHONPATH=${SimpleITK_Python_BINARY_DIR})
  endif()
...
function(sitk_add_lua_test name)
  ...
  sitk_add_test(
    ...
    ${command}
    ${ARGN})
  set_property(TEST Lua.${name} PROPERTY LABELS Lua)
  set_property(
    TEST Lua.${name}
    APPEND
...
function(sitk_add_ruby_test name)
  ...
  sitk_add_test(
    ...
    ${command}
    ${ARGN})
  set_property(TEST Ruby.${name} PROPERTY LABELS Ruby)
  set_property(
    TEST Ruby.${name}
    APPEND
...
function(sitk_add_tcl_test name)
  ...
  sitk_add_test(
    ...
    ${command}
    ${ARGN})
  set_property(TEST Tcl.${name} PROPERTY LABELS Tcl)
endfunction()
...
function(sitk_add_java_test name java_file)
  ...
  if(NOT TARGET ${_java_class}Java)
    ...
    add_custom_command(
      ...
      COMMAND "${CMAKE_COMMAND}" ARGS -E remove -f "${_class_path}/${_java_file_class}"
      COMMAND "${Java_JAVAC_EXECUTABLE}" ARGS -classpath "${_JAVA_CLASSPATH}" -d "${_class_path}" "${java_file}"
      DEPENDS ${java_file} ${SWIG_MODULE_SimpleITKJava_TARGET_NAME} org_itk_simple_jar
      COMMENT "Building ${_class_path}/${_java_file_class}")
    add_custom_target(
      ${_java_class}Java ALL
  ...
  sitk_add_test(
    ...
    "${_java_class}"
    ${ARGN})
  set_property(TEST Java.${name} PROPERTY LABELS Java)
endfunction()
...
function(sitk_add_r_test name)
  ...
  set_property(TEST R.${name} PROPERTY LABELS R)
...
#
# This is a function which compiles the program and set up the enviroment for executing CSharp examples and tests.
#
function(sitk_add_csharp_test name csharp_file)
  ...
  if(NOT TARGET "${CSHARP_EXECUTABLE}")
    ...
    csharp_add_executable("${CSHARP_EXECUTABLE}" SimpleITKCSharpManaged.dll ${csharp_file})
  endif()

  # because each executable is it's own target we actually don't need to make a target depend on this list
  ...
  set_property(TEST CSharp.${name} PROPERTY LABELS CSharp)
endfunction()
