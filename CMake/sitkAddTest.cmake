...
function(sitk_add_lua_test name)
  ...
  set_property(
    ...
    PROPERTY
      ENVIRONMENT
        LUA_CPATH=$<TARGET_FILE:SimpleITKLuaModule_LUA>
  )
endfunction()
...
function(sitk_add_ruby_test name)
  ...
  set_property(
    ...
    PROPERTY
      ENVIRONMENT
        RUBYLIB=$<TARGET_FILE_DIR:simpleitk_RUBY>
  )
endfunction()
...
function(sitk_add_java_test name java_file)
  ...
  if(WIN32)
    set(
      _JAVA_CLASSPATH
      "${SimpleITK_Java_BINARY_DIR}/${JAR_FILE}$<SEMICOLON>${_class_path}"
    )
  else()
    set(
      _JAVA_CLASSPATH
      "${SimpleITK_Java_BINARY_DIR}/${JAR_FILE}:${_class_path}"
    )
  endif()
  ...
  if(NOT TARGET ${_java_class}Java)
    add_custom_command(
      OUTPUT
        "${_class_path}/${_java_file_class}"
      COMMAND
        "${CMAKE_COMMAND}"
      ARGS
        -E remove -f "${_class_path}/${_java_file_class}"
      COMMAND
        "${Java_JAVAC_EXECUTABLE}"
      ARGS
        -classpath "${_JAVA_CLASSPATH}" -d "${_class_path}" "${java_file}"
      DEPENDS
        ${java_file}
...
function(sitk_add_csharp_test name csharp_file)
  ...
  if(CSHARP_INTERPRETER)
    sitk_add_test(
      ...
      COMMAND
        "${CSHARP_INTERPRETER}"
        "${CSHARP_BINARY_DIRECTORY}/${CSHARP_EXECUTABLE}.exe" ${ARGN}
    )
  else()
    sitk_add_test(
      NAME CSharp.${name}
