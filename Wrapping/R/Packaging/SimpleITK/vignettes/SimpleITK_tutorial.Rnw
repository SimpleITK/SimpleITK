...
Fetch SimpleITK from the git repository. Visit \url{https://simpleitk.readthedocs.io/en/latest/building.html} for the latest instructions on building and installing.
\section{Development}
