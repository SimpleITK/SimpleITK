...
Fetch SimpleITK from the git repository. Visit \url{http://www.itk.org/SimpleITKDoxygen/html/Wrapping.html} for the latest instructions on building and installing.
\section{Development}
