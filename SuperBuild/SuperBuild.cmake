...
if(PLATFORM_CHECK)
  # See CMake/Modules/Platform/Darwin.cmake) 6.x == Mac OSX 10.2 (Jaguar) 7.x == Mac OSX 10.3 (Panther) 8.x == Mac OSX
  # 10.4 (Tiger) 9.x == Mac OSX 10.5 (Leopard) 10.x == Mac OSX 10.6 (Snow Leopard)
...
#
# A function to get a filename for an ExternalData source file used in a superbuild. Adds a target which downloads all
# source code needed for superbuild projects. The source file is cached with in the build tree, and can be locally cache
# with other ExternalData controlled environment variables.
#
# <output variable> is set to the downloaded file. <output variable>_HASH is set to a hash of the file of the form
# ALGO=HASH_VALUE
#
# The "SuperBuildSimpleITKSource" target needs to be manually added as a dependencies to the ExternalProject.
#
...
function(sitkSourceDownload outVar filename)
  ...
  ExternalData_expand_arguments(SuperBuildSimpleITKSourceReal link DATA{${link_file}})
  set(${outVar}
      "${link}"
      PARENT_SCOPE)
endfunction()
...
set(SimpleITK_GIT_PROTOCOL
    ${_SimpleITK_GIT_PROTOCOL_default}
    CACHE STRING "If behind a firewall turn set this to 'https' or 'http'.")
...
if(NOT BUILDNAME)
  set(BUILDNAME
      "Unknown-build"
      CACHE STRING "Name of build to report to dashboard")
endif()
...
foreach(_varName ${_varNames})
  ...
  elseif(
    _varName MATCHES "^BUILD_DOCUMENTS$"
    OR _varName MATCHES "^BUILD_DOXYGEN$"
    OR _varName MATCHES "^DOXYGEN_"
    OR _varName MATCHES "^CMAKE_DISABLE_FIND_PACKAGE_"
    OR _varName
       MATCHES
    ...
    OR (_varName MATCHES "^\_Python\_VERSION$" AND CMAKE_VERSION VERSION_LESS "3.24")
    OR (_varName MATCHES "^\_Python\_VERSION\_(MAJOR|MINOR|PATCH)$" AND CMAKE_VERSION VERSION_LESS "3.24"))
    message(STATUS "Passing variable \"${_varName}=${${_varName}}\" to SimpleITK external project.")
    list(APPEND SimpleITK_VARS ${_varName})
  endif()
endforeach()
...
file(WRITE "${CMAKE_CURRENT_BINARY_DIR}/SimpleITK-build/CMakeCacheInit.txt"
...
ExternalProject_Add(
  ...
  DEPENDS ${${CMAKE_PROJECT_NAME}_DEPENDENCIES} ${External_Project_USES_TERMINAL}
...
ExternalProject_Add_Step(
  ${proj} doc
  COMMAND ${CMAKE_COMMAND} --build <BINARY_DIR> --target Documentation
  DEPENDEES configure
  EXCLUDE_FROM_MAIN 1
  LOG 1)
...
# We build SimpleITKExamples as an enternal project to verify installation of SimpleITK
